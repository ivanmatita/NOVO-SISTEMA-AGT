...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      let queryUrl = `${config.supabaseUrl}/rest/v1/movimentos_caixa?select=*&order=created_at.desc`;
    ...
    if (req.method === 'POST') {
      const body = req.body || {};
      const payload = { ...body, empresa_id: body.empresa_id || empresa_id };

      const insertRes = await fetch(`${config.supabaseUrl}/rest/v1/movimentos_caixa`, {
        method: 'POST',
        headers: {
          'apikey': config.serviceRoleKey,
          'Authorization': `Bearer ${config.serviceRoleKey}`,
