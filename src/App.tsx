      ...
      <div className="p-8 bg-zinc-50 min-h-screen">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          {[
            ...
            { id: 'transfer', label: 'Transferir Vendas', icon: ArrowRightLeft, color: 'bg-amber-500' },
            { id: 'fecho', label: 'Fecho de Caixa', icon: XCircle, color: 'bg-red-500', action: handleCloseSession },
            { id: 'stock', label: 'Ver Stock', icon: Package, color: 'bg-zinc-800' },
            { id: 'fechos_view', label: 'Ver Fechos', icon: ClipboardList, color: 'bg-zinc-700' },
            { id: 'permissions', label: 'Pertir Utilizador', icon: UserCheck, color: 'bg-blue-800' },
            { id: 'cash_move', label: 'Caixas Movimento', icon: Wallet, color: 'bg-emerald-700' },
            { id: 'occurrence', label: 'Adicionar Ocorrência', icon: AlertTriangle, color: 'bg-red-600' },
          ].map((item) => (
            <button
              key={item.id}
              onClick={() => {
