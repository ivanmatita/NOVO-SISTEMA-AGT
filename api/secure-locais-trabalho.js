...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const response = await fetch(`${config.supabaseUrl}/rest/v1/locais_trabalho?select=*&order=nome.asc`, {
        headers: {
    ...
    if (req.method === 'POST') {
      const response = await fetch(`${config.supabaseUrl}/rest/v1/locais_trabalho`, {
        method: 'POST',
        headers: {
          'apikey': config.serviceRoleKey,
          'Authorization': authHeader,
